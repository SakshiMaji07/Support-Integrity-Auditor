...
from predict import load_model, predict_batch, predict_ticket
...
if app_mode == "Executive Analytics Dashboard":
    st.markdown(
        '<div class="main-header">Executive Audit Dashboard</div>',
        unsafe_allow_html=True,
    )
    st.markdown(
        '<div class="sub-header">Analytical tracking visualizations detailing systematic priority-severity violations across customer support pipelines.</div>',
        unsafe_allow_html=True,
    )
    ...
    if predictions_path.exists():
        ...
        if "Prediction" not in df.columns and "Inferred_Severity" in df.columns:
            ...
            p_mapped = (
                df["Priority_Level"].map(PRIORITY_TO_SEVERITY_MAP).fillna(2)
            )
            df["Prediction"] = (np.abs(df["Inferred_Severity"] - p_mapped) >= 1).astype(
                int
            )
        ...
        if "mismatch_type" not in df.columns:
            ...
            s_col = (
                ...
                if "Inferred_Severity" in df.columns
                    ...
                    "Inferred Severity"
                    if "Inferred Severity" in df.columns
                    else "inferred_severity"
                )
            )

            for _, row in df.iterrows():
                m_type, delta = determine_mismatch_type(
                    row.get(p_col, "P3"), row.get(s_col, 2)
                )
                types.append(m_type)
                deltas.append(delta)
        ...
        with col2:
            st.metric(
                "Priority Mismatches Located",
                f"{total_mismatches:,}",
                delta=f"{mismatch_rate:.1f}% Rate",
                delta_color="inverse",
            )
        with col3:
            st.metric(
                "Hidden SLA Crisis Alerts",
                len(df[df["mismatch_type"] == "Hidden Crisis"]),
            )
        with col4:
        ...
        with g_col1:
            ...
            counts = df["Prediction"].value_counts().rename(
                index={0: "Consistent", 1: "Priority Mismatch"}
            )
            sns.barplot(
                x=counts.index,
                y=counts.values,
            ...
            if ch_col in df.columns:
                sns.countplot(
                    ...
                    x=ch_col,
                    hue="Prediction",
                    palette=["#A7F3D0", "#FCA5A5"],
                    ax=ax,
                )
                ax.set_title(
                    "Channel Distribution Across Alignment States", fontweight="bold"
                )
        ...
        with g_col2:
            ...
            if sig_cols:
                melted_signals = df.melt(
                    value_vars=sig_cols,
                    var_name="Signal Stream",
                    value_name="Calculated Density",
                )
                sns.boxplot(
                    data=melted_signals,
                    x="Signal Stream",
        ...
        st.warning(
            "Baseline evaluation database targets missing. Run individual audits or CSV drops to build system profiles."
        )
...
# -----------------------------------------------------------------------
    ...
    st.markdown(
        '<div class="main-header">Real-Time Ticket Investigator</div>',
        unsafe_allow_html=True,
    )
    st.markdown(
        '<div class="sub-header">Evaluate incoming specific consumer support issues to generate compliance evidence immediately.</div>',
        unsafe_allow_html=True,
    )
    ...
    with st.form("individual_ticket_form"):
        ...
        with col_t1:
            subject = st.text_input(
                "Ticket Subject Line",
                placeholder="e.g., Critical DB connection failure on master node",
            )
            channel = st.selectbox(
                "Inbound Communications Path (Channel)",
                ["Portal API", "Email Form", "Live Chat", "Phone Triage", "Premium Enterprise Hub"],
            )
        with col_t2:
            domain_tier = st.selectbox(
                "Client Service Plan Tier",
                ["Enterprise Premium", "Strategic Partner", "Mid-Market Corporate", "Standard Tier Growth"],
            )
            assigned_priority = st.selectbox(
                "Assigned Ticket Priority Level", ["P1", "P2", "P3", "P4"]
            )

        description = st.text_area(
            "Comprehensive Ticket Body Description",
            placeholder="Enter the customer payload log errors or operational updates completely...",
        )
    ...
    if submit_btn:
        if not subject.strip() or not description.strip():
            ...
            ticket_payload = {
                ...
                "Inferred Severity": int(
                    round(fused_sev * 3 + 1)
                ),  # Transpose 0-1 scale back to 1-4
                "Ticket_ID": "SIA-LIVE-MOCK-REGISTRY",
            }

            with st.spinner(
                "Processing text configurations and scaling multidimensional embeddings..."
            ):
            ...
            st.markdown(
                "### Grounded Dossier Evidence Dossier Document (JSON Compliance Standard)"
            )
            ...
            mock_series = pd.Series(
                {
                    ...
                    "resolution_severity": res_sev,
                    "cluster_severity": clu_sev,
                    "fused_severity": fused_sev,
                    "Priority_Level": assigned_priority,
                    "Inferred_Severity": int(round(fused_sev * 3 + 1)),
                    "Confidence": confidence_score,
                    "Ticket_ID": "SIA-AUDIT-LOG-LN",
                }
            )
...
# -----------------------------------------------------------------------
    ...
    st.markdown(
        '<div class="main-header">High-Throughput Batch Auditing</div>',
        unsafe_allow_html=True,
    )
    st.markdown(
        '<div class="sub-header">Drop entire CSV operational blocks to pass metrics parameters down the fine-tuned system.</div>',
        unsafe_allow_html=True,
    )
    ...
    if uploaded_file is not None:
        try:
            ...
            st.success(
                f"Successfully mounted file stream. Located `{len(raw_batch_df)}` rows ready for evaluation."
            )
            ...
            missing_cols = [
                col
                for col in required_options
                if col not in raw_batch_df.columns
                and col.replace(" ", "_") not in raw_batch_df.columns
            ]

            if missing_cols:
                st.warning(
                    f"Schema warning: Missing core column headers {missing_cols}. Missing keys will fall back to pipeline default metrics."
                )

            if st.button("Execute Stream Batch Classification"):
                with st.spinner(
                    "Processing deep network inference sweeps over chunk distributions..."
                ):
                    # Invoke production batch mapping functions from predict.py
                    results_df = predict_batch(
                        raw_batch_df, model, tokenizer, artifacts, batch_size=32
                    )

                st.success(
                    "Batch classification complete! Tracking updates saved to `outputs/predictions.csv`."
                )

                # Dynamic summary metrics rendering
                mismatch_count = (results_df["Prediction"] == 1).sum()
                st.markdown(
                    f"**Audit Profile Results:** Found **{mismatch_count}** anomalies out of **{len(results_df)}** entries."
                )
