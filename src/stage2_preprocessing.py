...
def run_stage2(
    ...
    UPDATED: Now uses processed.csv as primary ticket data source (with combined_text),
    and optionally merges pseudo-labels for annotations.

    Args:
        ticket_data_path (str | Path | None): Path to processed.csv (PRIMARY text source with combined_text).
    ...
    logger.info("Data flow: PRIMARY=[processed.csv with combined_text] + SECONDARY=[pseudo_labels.csv for annotations]")

    # Establish localized target workspace layouts
    project_root = Path(__file__).resolve().parent
    if ticket_data_path is None:
        # PRIMARY data source: use processed.csv (contains combined_text)
    ...
    if not ticket_data_path.exists():
        ...
        logger.error(f"Expected processed.csv with combined_text column.")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="[Stage 2] Path to PRIMARY ticket data source (processed.csv with combined_text).",
    )
    parser.add_argument(
        "--metrics-output-path",
        type=str,
